...
export default async function AnalyticsPage() {
  ...
  const openCount = requests.filter((request) => !["resolved", "closed"].includes(request.status)).length;
  ...
  const priorityCount = requests.filter((request) => ["high", "critical"].includes(request.priority) && !["resolved", "closed"].includes(request.status)).length;
  const maxCount = Math.max(...statuses.map((item) => item.count), ...categories.map((item) => item.count), ...workloadEntries.map(([, count]) => count), 1);
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[["Open Cases", openCount, "Cases requiring action"], ["In Progress", inProgressCount, "Actively being worked"], ["Resolved", resolvedCount, "Resolved or closed cases"], ["High Priority", priorityCount, "Open high or critical cases"]].map(([label, value, detail]) => <Card key={label as string}><CardHeader className="pb-2"><CardDescription className="text-xs font-semibold uppercase tracking-wide">{label}</CardDescription><CardTitle className="text-3xl font-bold tabular-nums">{value}</CardTitle></CardHeader><CardContent><p className="text-xs text-muted-foreground">{detail}</p></CardContent></Card>)}</div>
    <div className="grid gap-6 xl:grid-cols-2">
      <Card><CardHeader className="border-b border-border/60"><CardTitle>Cases by Status</CardTitle><CardDescription>Current distribution across the support lifecycle.</CardDescription></CardHeader><CardContent className="space-y-4 pt-5">{statuses.length ? statuses.map(({ status, count }) => <div key={status} className="grid grid-cols-[auto_1fr_auto] items-center gap-3"><Badge variant={statusVariant(status)}>{formatCaseLabel(status)}</Badge><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary/75" style={{ width: `${(count / maxCount) * 100}%` }} /></div><span className="w-6 text-right text-sm font-semibold tabular-nums">{count}</span></div>) : <p className="text-sm text-muted-foreground">No case data is available yet.</p>}</CardContent></Card>
      <Card><CardHeader className="border-b border-border/60"><CardTitle>Cases by Category</CardTitle><CardDescription>Where the current workload is concentrated.</CardDescription></CardHeader><CardContent className="space-y-4 pt-5">{categories.length ? categories.map(({ category, count }) => <div key={category} className="grid grid-cols-[9rem_1fr_auto] items-center gap-3"><span className="truncate text-sm font-medium" title={formatCategoryLabel(category)}>{formatCategoryLabel(category)}</span><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-blue-500/70" style={{ width: `${(count / maxCount) * 100}%` }} /></div><span className="w-6 text-right text-sm font-semibold tabular-nums">{count}</span></div>) : <p className="text-sm text-muted-foreground">No case data is available yet.</p>}</CardContent></Card>
    </div>
